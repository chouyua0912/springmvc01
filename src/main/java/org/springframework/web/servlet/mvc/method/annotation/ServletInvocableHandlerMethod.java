...
public class ServletInvocableHandlerMethod extends InvocableHandlerMethod {
    ...
    public void invokeAndHandle(ServletWebRequest webRequest, ModelAndViewContainer mavContainer,
        ...
        setResponseStatus(webRequest);          // 处理 ResponseStatus 注解设置的HttpStatus返状态

        if (returnValue == null) {          // 方法没有返回值
            if (isRequestNotModified(webRequest) || getResponseStatus() != null || mavContainer.isRequestHandled()) {
                mavContainer.setRequestHandled(true);
                return;
            }
        }
        else if (StringUtils.hasText(getResponseStatusReason())) {  // 获取ResponseStatus注解的reason
             // text
            mavContainer.setRequestHandled(true);
            return;
        }
        ...
        try {
            this.returnValueHandlers.handleReturnValue(             // 处理方法的返回值
